...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            {{-- Cambiamos la acción según si es crear o editar --}}
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nombre" class="form-control input-dark" 
                                       placeholder="Ej. Control de Roedores" required 
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="area" class="form-control input-dark" 
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                @php $nombreRelacion = 'rel' . ucfirst($pr); @endphp
                                
                                {{-- Input VISIBLE: Ahora tiene su propio name para guardar el texto en caso de error --}}
                                <input type="text" 
                                    name="{{ $pr }}_nombre" 
                                    data-url="{{ route('productos.buscar') }}"
                                    class="form-control input-dark buscar-producto" 
                                    placeholder="Escribe..."
                                    value="{{ old($pr.'_nombre', $actividad->$nombreRelacion->nombre ?? '') }}">

                                {{-- Input OCULTO: Guarda el ID para la base de datos --}}
                                <input type="hidden" name="{{ $pr }}" value="{{ old($pr, $actividad->{$pr} ?? '') }}">
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                @php $nombreRelacion = 'rel' . ucfirst($tec); @endphp
                                ...
                                <input type="text" 
                                    ...
                                    placeholder="Nombre..."
                                    value="{{ old($tec.'_nombre', $actividad->$nombreRelacion->nombre ?? '') }}">

                                <input type="hidden" name="{{ $tec }}" value="{{ old($tec, $actividad->{$tec} ?? '') }}">
                            </div>
                            @endforeach
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <textarea name="observacion" class="form-control input-dark" rows="5" 
                                          placeholder="Detalles...">{{ old('observacion', $actividad->observacion ?? '') }}</textarea>
                            </div>
                            <div class="col-md-6 text-center">
                                ...
                                <div class="foto-upload-zone" onclick="document.getElementById('foto').click()">
                                    {{-- El placeholder se oculta si ya existe una foto --}}
                                    <div id="uploadPlaceholder" style="{{ isset($actividad->foto) ? 'display: none;' : '' }}">
                                        <i class="fa-solid fa-camera fa-2x mb-2 opacity-50"></i>
                                        <p class="small mb-0">Haz clic para subir foto</p>
                                    </div>
                                    
                                    {{-- La imagen se muestra si existe en storage --}}
                                    <img id="preview" 
                                         src="{{ isset($actividad->foto) ? asset('storage/' . $actividad->foto) : '' }}" 
                                         class="preview-img mx-auto" 
                                         style="{{ isset($actividad->foto) ? 'display: block;' : 'display: none;' }}">
                                </div>
                        ...
                        <div class="d-flex gap-2 mt-5">
                            <a href="/actividades/{{ $id_servicio }}" class="btn w-50" 
                               style="background-color: #2c1a1a; color: #d66d6d; border: 1px solid #4a2424;">
                                Cancelar
                            </a>
                            <button type="submit" class="btn w-50" 
                                    style="background-color: #1c2a35; color: #6dacd6; border: 1px solid #243b4a;">
